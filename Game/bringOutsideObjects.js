...
// Read a file
function readOBJFile(obj, fileName, gl, model, scale, reverse) {
	var request = new XMLHttpRequest();

	request.onreadystatechange = function() {
	  if (request.readyState === 4 && request.status !== 404) {
	    onReadOBJFile(obj, request.responseText, fileName, gl, model, scale, reverse);
	  }
	}
...
function onReadOBJFile(obj, fileString, fileName, gl, o, scale, reverse) {
	var objDoc = new OBJDoc(fileName);  // Create a OBJDoc object
	var result = objDoc.parse(fileString, scale, reverse); // Parse the file
	if (!result) {
	  obj.g_objDoc = null; obj.g_drawingInfo = null;
